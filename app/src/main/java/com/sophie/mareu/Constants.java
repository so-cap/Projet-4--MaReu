package com.sophie.mareu;

public class Constants {
    public static final int ASCENDING = 0;
    public static final int DESCENDING = 1;
    public static final int FILTERED = 0;
    public static final int SORTED = 1;
    public static final int UNCHANGED = -1;

    public static final String ARGUMENT_MEETING = "selected meeting";
    public static final String ARGUMENT_HOUR_KEY = "selected hour key";
    public static final String ARGUMENT_HOUR_VALUE = "selected hour value";
    public static final String ARGUMENT_ROOM = "selected room";
    public static final String ARGUMENT_HOUR_POSITION = "hour position";
    public static final String ARGUMENT_ROOM_POSITION = "room position";
    public static final String ARGUMENT_DATE = "date";
    public static final String ARGUMENT_SERVICE = "rooms availability service";
}
